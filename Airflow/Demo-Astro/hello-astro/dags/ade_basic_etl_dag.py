from airflow.models import dagparam
import json
...
def getSampleValue(sample_value):
    level_1_value_type = type(sample_value)
    if isinstance(sample_value, dict):
        for k, v in sample_value.items():
            print(f"key={k}\tvalue={v}")
    else:
        print(f"sample_value type={type(sample_value)}")
        level_2_value = sample_value.params
        print(f"level_2_value={level_2_value}")

DAG_ID = "ade_basic_etl_dag"
TIME_DELAY_SECS = 10

def runDag(sample_value=100):
    ...
    def ade_dag_basic():
        ...
        def extract():
            ...
            """
            # current_sample_value = kwargs['dag_run'].conf['sample_value']
            current_sample_value = sample_value
            # getSampleValue(current_sample_value)
            print("============================================")
            print(f"Sample value is: {current_sample_value}")
            print("============================================")
            print(f"Running extract task, this will take {TIME_DELAY_SECS} seconds")
        ...
        def transform(order_data_dict: dict):
            ...
            #print(f"Sample value is: {current_sample_value}")
            print("============================================")
            print(f"Running transform task, this will take {TIME_DELAY_SECS} seconds")
            time.sleep(TIME_DELAY_SECS)
            total_order_value = 0
        ...
        def load(total_order_value: float):
            ...
            #print(f"Sample value is: {current_sample_value}")
            print("============================================")
            print(f"Running load task, this will take {TIME_DELAY_SECS} seconds")
            time.sleep(TIME_DELAY_SECS)
